...
import com.kapstranspvtltd.kaps_partner.databinding.ActivityJcbCraneAgentPucuploadBinding;
...
public class JcbCraneAgentPUCUploadActivity extends AppCompatActivity {

    private static final String TAG = "CabDriverPUCUploadActivity";
    private static final int CAMERA_PERMISSION_REQUEST = 100;

    private ActivityJcbCraneAgentPucuploadBinding binding;
    private PreferenceManager preferenceManager;
    private ExecutorService cameraExecutor;
    private ImageCapture imageCapture;
    private Uri pucImageUri;
    private String previousPUC;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivityJcbCraneAgentPucuploadBinding.inflate(getLayoutInflater());
    ...
    private void setupUI() {
        ...
        binding.pucContainer.setOnClickListener(v -> showCameraPreview(false));
        binding.updateButton.setOnClickListener(v -> savePUCDetails());
    }

    private void loadSavedData() {
        previousPUC = preferenceManager.getStringValue("jcb_crane_agent_puc_photo_url");
        binding.pucNumberInput.setText(preferenceManager.getStringValue("jcb_crane_agent_puc_no"));
    ...
    private void savePUCDetails() {
        String pucNo = binding.pucNumberInput.getText().toString().trim();
        ...
        if (isNewEntry() && pucImageUri == null) {
            showToast("Please select and upload your PUC certificate image");
            return;
        }

        if (needsImageUpload() && pucImageUri != null) {
            uploadImage(pucImageUri);
        } else {
            if (!needsImageUpload() && pucNo.equals(preferenceManager.getStringValue("jcb_crane_agent_puc_no"))) {
                showToast("No changes made");
                return;
            }
            preferenceManager.saveStringValue("jcb_crane_agent_puc_no", pucNo);
    ...
    private void updatePreferencesAndUI(String imageUrl) {
        preferenceManager.saveStringValue("jcb_crane_agent_puc_photo_url", imageUrl);
        previousPUC = imageUrl;
        loadImage(imageUrl, binding.pucImage, binding.pucPlaceholder);
    }

    private boolean isNewEntry() {
        return preferenceManager.getStringValue("jcb_crane_agent_puc_no").isEmpty() &&
                (previousPUC == null || previousPUC.isEmpty());
    }
    ...
    private boolean needsImageUpload() {
        return isNewEntry() ||
                (pucImageUri != null &&
                        (previousPUC == null ||
                                !previousPUC.equals(pucImageUri.toString())));
    }
