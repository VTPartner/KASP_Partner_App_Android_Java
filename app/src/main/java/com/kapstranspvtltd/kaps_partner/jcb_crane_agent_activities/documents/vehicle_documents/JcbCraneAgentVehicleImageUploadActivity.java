...
import com.kapstranspvtltd.kaps_partner.databinding.ActivityJcbCraneAgentVehicleImageUploadBinding;
...
public class JcbCraneAgentVehicleImageUploadActivity extends AppCompatActivity {

    private static final String TAG = "CabDriverVehicleImageUploadActivity";
    private static final int CAMERA_PERMISSION_REQUEST = 100;

    private ActivityJcbCraneAgentVehicleImageUploadBinding binding;
    private PreferenceManager preferenceManager;
    private ExecutorService cameraExecutor;
    private ImageCapture imageCapture;
    private Uri vehicleFrontUri;
    private Uri vehicleBackUri;
    private String previousVehicleFront;
    private String previousVehicleBack;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivityJcbCraneAgentVehicleImageUploadBinding.inflate(getLayoutInflater());
    ...
    private void setupUI() {
        ...
        binding.vehicleFrontContainer.setOnClickListener(v -> showCameraPreview(true));
        binding.vehicleBackContainer.setOnClickListener(v -> showCameraPreview(false));
        binding.updateButton.setOnClickListener(v -> saveVehicleDetails());
    }

    private void loadSavedData() {
        previousVehicleFront = preferenceManager.getStringValue("jcb_crane_agent_vehicle_front_photo_url");
        previousVehicleBack = preferenceManager.getStringValue("jcb_crane_agent_vehicle_back_photo_url");
    ...
    private void saveVehicleDetails() {
        if (isNewEntry() && vehicleFrontUri == null) {
            showToast("Please select and upload your vehicle front image");
            return;
        }
        if (isNewEntry() && vehicleBackUri == null) {
            showToast("Please select and upload your vehicle back image");
            return;
        }

        if (needsFrontUpload() && vehicleFrontUri != null) {
            uploadImage(vehicleFrontUri, true);
        }

        if (needsBackUpload() && vehicleBackUri != null) {
            uploadImage(vehicleBackUri, false);
        }
    ...
    private String createFileName(boolean isFront) {
        ...
        return "goods_driver_id_" + driverId + "_" + driverName + "_vehicle_" +
    ...
    private void updatePreferencesAndUI(String imageUrl, boolean isFront) {
        if (isFront) {
            preferenceManager.saveStringValue("jcb_crane_agent_vehicle_front_photo_url", imageUrl);
            previousVehicleFront = imageUrl;
            loadImage(imageUrl, binding.vehicleFrontImage, binding.vehicleFrontPlaceholder);
        } else {
            preferenceManager.saveStringValue("jcb_crane_agent_vehicle_back_photo_url", imageUrl);
    ...
    private boolean isNewEntry() {
        return (previousVehicleFront == null || previousVehicleFront.isEmpty()) ||
                (previousVehicleBack == null || previousVehicleBack.isEmpty());
    }

    private boolean needsFrontUpload() {
        return isNewEntry() ||
                (vehicleFrontUri != null &&
                        (previousVehicleFront == null ||
                                !previousVehicleFront.equals(vehicleFrontUri.toString())));
    }
        ...
        return isNewEntry() ||
                (vehicleBackUri != null &&
                        (previousVehicleBack == null ||
                                !previousVehicleBack.equals(vehicleBackUri.toString())));
    }
