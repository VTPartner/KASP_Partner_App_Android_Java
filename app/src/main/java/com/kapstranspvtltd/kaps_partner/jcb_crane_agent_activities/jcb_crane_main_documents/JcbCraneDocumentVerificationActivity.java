...
import com.kapstranspvtltd.kaps_partner.databinding.ActivityJcbCraneDocumentVerificationBinding;

import com.kapstranspvtltd.kaps_partner.goods_driver_activities.documents.main_document_screens.GoodsDriverVehicleDocumentsVerificationActivity;
import com.kapstranspvtltd.kaps_partner.handyman_agent_activities.HandyManAgentHomeActivity;
import com.kapstranspvtltd.kaps_partner.jcb_crane_agent_activities.documents.JcbCraneAgentAadharCardUploadActivity;
import com.kapstranspvtltd.kaps_partner.jcb_crane_agent_activities.documents.JcbCraneAgentDrivingLicenseUploadActivity;
import com.kapstranspvtltd.kaps_partner.jcb_crane_agent_activities.documents.JcbCraneAgentPanCardUploadActivity;
import com.kapstranspvtltd.kaps_partner.jcb_crane_agent_activities.documents.JcbCraneAgentSelfieUploadActivity;
...
public class JcbCraneDocumentVerificationActivity extends AppCompatActivity {

    private static final String TAG = "JcbCraneDocumentVerificationActivity";
    private ActivityJcbCraneDocumentVerificationBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityJcbCraneDocumentVerificationBinding.inflate(getLayoutInflater());
                        ...
                        try {
                            if (response.has("results")) {
                                ...
                                if (!serviceNames.isEmpty()) {
                                    binding.otherServiceDropdownTextInput.setVisibility(View.VISIBLE);
                                    ...
                                    binding.otherServiceDropdownTextInput.setVisibility(View.GONE);
                                    selectedServiceId = "-1";
    ...
    private void setupUI() {
        ...
        binding.drivingLicenseItem.setOnClickListener(v ->
                startActivity(new Intent(this, JcbCraneAgentDrivingLicenseUploadActivity.class)));

        binding.aadharCardItem.setOnClickListener(v ->
                startActivity(new Intent(this, JcbCraneAgentAadharCardUploadActivity.class)));

        binding.panCardItem.setOnClickListener(v ->
                startActivity(new Intent(this, JcbCraneAgentPanCardUploadActivity.class)));

        binding.selfieItem.setOnClickListener(v ->
                startActivity(new Intent(this, JcbCraneAgentSelfieUploadActivity.class)));

        binding.continueButton.setOnClickListener(v -> saveDriverDetails());

        // Restore saved values if any
        binding.nameInput.setText(preferenceManager.getStringValue("jcb_crane_agent_name"));
        binding.addressInput.setText(preferenceManager.getStringValue("jcb_crane_agent_full_address"));
        String savedGender = preferenceManager.getStringValue("jcb_crane_agent_gender");
        ...
        JsonObjectRequest request = new JsonObjectRequest(
                ...
                response -> {
                    try {
                        ...
                        String savedCityId = preferenceManager.getStringValue("jcb_crane_agent_city_id");
    ...
    private void saveDriverDetails() {
        ...
        if (selectedSubCategoryId.equalsIgnoreCase("-1")) {
            showError("Please select your service speciality");
            return;
        }
        if (!checkDocuments()) {
            return;
        ...
        preferenceManager.saveStringValue("jcb_crane_agent_name", name.trim());
        preferenceManager.saveStringValue("jcb_crane_agent_full_address", address.trim());
        preferenceManager.saveStringValue("jcb_crane_agent_gender", selectedGender);
        preferenceManager.saveStringValue("jcb_crane_agent_city_id", selectedCityId);
        preferenceManager.saveStringValue("jcb_crane_agent_sub_category_id", selectedSubCategoryId);
        preferenceManager.saveStringValue("jcb_crane_agent_other_service_id", selectedServiceId);

        //Navigate to Vehicle Information Screen
        startActivity(new Intent(this, JcbCraneVehicleDocumentsVerificationActivity.class));
    }
    ...
    private boolean checkDocuments() {
        if (preferenceManager.getStringValue("jcb_crane_agent_license_front_photo_url").isEmpty()) {
            showError("Please upload Driving License Front Picture");
            return false;
        }
        if (preferenceManager.getStringValue("jcb_crane_agent_license_back_photo_url").isEmpty()) {
            showError("Please upload Driving License Back Picture");
            return false;
        }
        if (preferenceManager.getStringValue("jcb_crane_agent_license_no").isEmpty()) {
            showError("Please upload Driving License Number");
            return false;
        }
        if (preferenceManager.getStringValue("jcb_crane_agent_aadhar_front_photo_url").isEmpty()) {
            showError("Please upload Aadhar Front Picture");
            return false;
        }
        if (preferenceManager.getStringValue("jcb_crane_agent_aadhar_back_photo_url").isEmpty()) {
            showError("Please upload Aadhar Back Picture");
            return false;
        }
        if (preferenceManager.getStringValue("jcb_crane_agent_aadhar_no").isEmpty()) {
            showError("Please provide your Aadhar Number");
            return false;
        }
        if (preferenceManager.getStringValue("jcb_crane_agent_pan_no").isEmpty()) {
            showError("Please upload PAN Number");
            return false;
        }
        if (preferenceManager.getStringValue("jcb_crane_agent_pan_front_photo_url").isEmpty()) {
            showError("Please upload PAN Front Picture");
            return false;
        }
        if (preferenceManager.getStringValue("jcb_crane_agent_pan_back_photo_url").isEmpty()) {
            showError("Please upload PAN Back Picture");
            return false;
        }
        if (preferenceManager.getStringValue("jcb_crane_agent_selfie_photo_url").isEmpty()) {
