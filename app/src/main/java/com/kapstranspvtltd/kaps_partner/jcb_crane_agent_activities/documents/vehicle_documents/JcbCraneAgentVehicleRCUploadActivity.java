...
import com.kapstranspvtltd.kaps_partner.databinding.ActivityCabDriverVehicleRcuploadBinding;
import com.kapstranspvtltd.kaps_partner.databinding.ActivityJcbCraneAgentVehicleRcuploadBinding;
...
public class JcbCraneAgentVehicleRCUploadActivity extends AppCompatActivity {

    private static final String TAG = "CabDriverRCUpload";
    private static final int CAMERA_PERMISSION_REQUEST = 100;

    private ActivityCabDriverVehicleRcuploadBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityCabDriverVehicleRcuploadBinding.inflate(getLayoutInflater());
    ...
    private void setupUI() {
        ...
        binding.rcContainer.setOnClickListener(v -> showCameraPreview(false));
        binding.updateButton.setOnClickListener(v -> saveRCDetails());
    }

    private void loadSavedData() {
        previousRC = preferenceManager.getStringValue("jcb_crane_agent_rc_photo_url");
        binding.rcNumberInput.setText(preferenceManager.getStringValue("jcb_crane_agent_rc_no"));
    ...
    private void saveRCDetails() {
        String rcNo = binding.rcNumberInput.getText().toString().trim();
        ...
        if (isNewEntry() && rcImageUri == null) {
            showToast("Please select and upload your RC certificate image");
            return;
        }

        if (needsImageUpload() && rcImageUri != null) {
            uploadImage(rcImageUri);
        } else {
            if (!needsImageUpload() && rcNo.equals(preferenceManager.getStringValue("jcb_crane_agent_rc_no"))) {
                showToast("No changes made");
                return;
            }
            preferenceManager.saveStringValue("jcb_crane_agent_rc_no", rcNo);
            showToast("RC details saved successfully");
            finish();
        }
    }
    ...
    private void uploadImage(Uri uri) {
        Dialog loadingDialog = new MaterialAlertDialogBuilder(this)
                .setTitle("Uploading RC Certificate")
    ...
    private void handleUploadSuccess(NetworkResponse response, Dialog dialog) {
        try {
            ...
            showToast("RC certificate uploaded successfully");
            finish();
        } catch (Exception e) {
    ...
    private void updatePreferencesAndUI(String imageUrl) {
        preferenceManager.saveStringValue("jcb_crane_agent_rc_photo_url", imageUrl);
        previousRC = imageUrl;
        loadImage(imageUrl, binding.rcImage, binding.rcPlaceholder);
    }

    private boolean isNewEntry() {
        return preferenceManager.getStringValue("jcb_crane_agent_rc_no").isEmpty() &&
                (previousRC == null || previousRC.isEmpty());
    }
    ...
    private boolean needsImageUpload() {
        return isNewEntry() ||
                (rcImageUri != null &&
                        (previousRC == null ||
                                !previousRC.equals(rcImageUri.toString())));
    }
