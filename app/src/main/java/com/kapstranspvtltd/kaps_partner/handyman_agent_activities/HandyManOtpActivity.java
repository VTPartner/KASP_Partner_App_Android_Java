...
import com.kapstranspvtltd.kaps_partner.databinding.ActivityHandyManOtpBinding;
...
import com.kapstranspvtltd.kaps_partner.handyman_agent_activities.documents.handyman_main_documents.HandyManDocumentVerificationActivity;
...
public class HandyManOtpActivity extends AppCompatActivity {

    private ActivityHandyManOtpBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityHandyManOtpBinding.inflate(getLayoutInflater());
    ...
    private void verifyOTPAndLogin() {
        showLoading();
        try {
            String url = APIClient.baseUrl + "handyman_login";
        ...
        try {
            if (response.has("results") && !response.isNull("results")) {
                ...
                if (result.length() > 0) {
                    ...
                    preferenceManager.saveStringValue("handyman_agent_mobile_no", countryCode + mobileNumber);
                    preferenceManager.saveStringValue("handyman_agent_id", user.optString("handyman_id"));
    ...
    private void saveUserDetails(JSONObject user) {
        preferenceManager.saveStringValue("handyman_agent_id", user.optString("handyman_id"));
        preferenceManager.saveStringValue("handyman_agent_name", user.optString("driver_first_name"));
        preferenceManager.saveStringValue("profile_pic", user.optString("profile_pic"));
        preferenceManager.saveStringValue("handyman_agent_mobile_no", countryCode + mobileNumber);
        preferenceManager.saveStringValue("handyman_agent_full_address", user.optString("full_address"));
    }
    ...
    private void navigateToRegistration() {
        Intent intent = new Intent(this, HandyManDocumentVerificationActivity.class);
        startActivity(intent);
        finish();
    }
    ...
    private void handleError(Exception e) {
        e.printStackTrace();
        preferenceManager.saveStringValue("handyman_agent_id", "");
        preferenceManager.saveStringValue("handyman_agent_name", "");
    ...
    private void checkPermissionsAndProceed() {
        if (areAllPermissionsGranted()) {
            String goodsDriverID = preferenceManager.getStringValue("handyman_agent_id");
            String goodsDriverName = preferenceManager.getStringValue("handyman_agent_name");
            ...
            if (goodsDriverID != null && !goodsDriverID.isEmpty() &&
                ...
                startActivity(new Intent(this, HandyManDocumentVerificationActivity.class));
            }
        } else {
