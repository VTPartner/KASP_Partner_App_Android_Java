...
import com.kapstranspvtltd.kaps_partner.cab_driver_activities.documents.main_documents.CabDriverDocumentVerificationActivity;
...
import com.kapstranspvtltd.kaps_partner.databinding.ActivityCabOtpVerificationBinding;
...
public class CabOtpVerificationActivity extends AppCompatActivity {

    private ActivityCabOtpVerificationBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityCabOtpVerificationBinding.inflate(getLayoutInflater());
    ...
    private void verifyOTPAndLogin() {
        showLoading();
        try {
            String url = APIClient.baseUrl + "cab_driver_login";
        ...
        try {
            if (response.has("results") && !response.isNull("results")) {
                ...
                if (result.length() > 0) {
                    ...
                    preferenceManager.saveStringValue("cab_driver_mobile_no", countryCode + mobileNumber);
                    preferenceManager.saveStringValue("cab_driver_id", user.optString("cab_driver_id"));
    ...
    private void saveUserDetails(JSONObject user) {
        preferenceManager.saveStringValue("cab_driver_id", user.optString("cab_driver_id"));
        preferenceManager.saveStringValue("cab_agent_driver_name", user.optString("driver_first_name"));
        preferenceManager.saveStringValue("cab_driver_profile_pic", user.optString("profile_pic"));
        preferenceManager.saveStringValue("cab_driver_mobile_no", countryCode + mobileNumber);
        preferenceManager.saveStringValue("cab_driver_full_address", user.optString("full_address"));
    }
    ...
    private void navigateToRegistration() {
        Intent intent = new Intent(this, CabDriverDocumentVerificationActivity.class);
        startActivity(intent);
        finish();
    }
    ...
    private void checkPermissionsAndProceed() {
        if (areAllPermissionsGranted()) {
            ...
            if (cabDriverID != null && !cabDriverID.isEmpty() &&
                ...
                startActivity(new Intent(this, CabDriverDocumentVerificationActivity.class));
            }
        } else {
